...
export default function AndroidPricing1() {
  const [annual, setAnnual] = useState(false);

  const plans = [
    {
      name: "Free", btn: "bg-gray-100 text-gray-700",
      price: "$0", period: "forever",
      features: ["5 projects", "1 GB storage", "Basic sync", "Email support"],
    },
    {
      name: "Pro", btn: "bg-white text-purple-600", highlight: true,
      price: annual ? "$2.49" : "$3.99",
      period: annual ? "per month, billed yearly" : "per month",
      features: ["Unlimited projects", "50 GB storage", "Drive sync", "Priority support", "Advanced analytics", "Custom themes"],
    },
  ];
    ...
    <section className="py-16 px-6 bg-white" style={{ fontFamily: "'Google Sans','Roboto',sans-serif" }}>
      <div className="max-w-sm mx-auto">
        ...
        <div className="flex flex-col gap-4">
          {plans.map(plan => (
            <div key={plan.name} className={`rounded-3xl p-6 ${plan.highlight ? "bg-purple-600 shadow-2xl shadow-purple-200" : "border border-gray-100 shadow-sm bg-white"}`}>
              {plan.highlight && <span className="text-xs font-bold text-purple-200 uppercase tracking-widest">Most Popular</span>}
              <div className="flex items-end gap-1 mt-1 mb-1">
                ...
                <span className={`text-sm pb-1.5 ${plan.highlight ? "text-purple-200" : "text-gray-400"}`}>{plan.period}</span>
              </div>
              {plan.highlight && annual && <p className="text-purple-200 text-xs mb-4">Billed as ${(2.49 * 12).toFixed(2)}/year · Save $18</p>}
              <ul className={`space-y-2.5 mb-6 ${plan.highlight && annual ? "" : "mt-5"}`}>
                {plan.features.map(f => (
                  <li key={f} className={`flex items-center gap-2.5 text-sm ${plan.highlight ? "text-white" : "text-gray-700"}`}>
                    <span className={`text-base ${plan.highlight ? "text-purple-200" : "text-green-500"}`}>✓</span>{f}
                  </li>
                ))}
              </ul>
        ...
        <p className="text-center text-xs text-gray-400 mt-5">Cancel anytime · Billed via Google Play</p>
      </div>
    </section>
  );
}
