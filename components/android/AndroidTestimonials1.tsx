...
export default function AndroidTestimonials1() {
  return (
    <section className="py-16 px-6 bg-gray-50" style={{ fontFamily: "'Google Sans','Roboto',sans-serif" }}>
      <div className="max-w-lg mx-auto">
        ...
        <div className="flex flex-col gap-4">
          {reviews.map(r => (
            <div key={r.name} className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100">
        ...
        <div className="mt-6 bg-white rounded-3xl p-4 flex items-center justify-between border border-gray-100 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-2xl flex items-center justify-center text-2xl shadow-md">✦</div>
            <div>
              <p className="font-semibold text-gray-900 text-sm">Get it on Google Play</p>
              <div className="flex items-center gap-1 mt-0.5">
                <span className="text-yellow-400 text-xs">★★★★★</span>
                <span className="text-xs text-gray-400">30K ratings</span>
              </div>
            </div>
          </div>
          <button className="bg-purple-600 text-white text-sm font-semibold px-4 py-2 rounded-2xl">Install</button>
        </div>
      </div>
    </section>
  );
